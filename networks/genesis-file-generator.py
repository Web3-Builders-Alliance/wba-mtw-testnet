...
CHAIN_ID = "eve-v1"
GENESIS_FILE=f"{Path.home()}/.wbad/config/genesis.json" # .eve in future
...
CUSTOM_GENESIS_ACCOUNT_VALUES = {
    ...
    "eve1hj5fveer5cjtn4wd6wstzugjfdxzl0xpysfwwn": "2000000uwba # note here", # useful to auto gen extra for non gentx accounts, or to give them more
}
...
def createGenesisAccountsCommands():
    ...
    # give validators their amounts in the genesis (1uwba, or more if provided in the custom dict)
    for file in gentx_files:
        ...
        if val_addr not in CUSTOM_GENESIS_ACCOUNT_VALUES.keys():
            print(f"eve add-genesis-account {val_addr} {amt}uwba #{moniker}")
            continue # 
